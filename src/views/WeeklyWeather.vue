<template>
  <div class="container-fluid px-5">
    ...
    <div class="row">
      <div
        v-for="forecast in formatForecast"
        :key="forecast.number"
        class="card col"
        style="width: 18rem"
      >
...
const formatForecast = computed(() => {
  ...
  for (let i = 0; i < weatherStore.weeklyForecast.length; i++) {
    ...
    if (previousDate === null) {
      if (
        new Date(
          weatherStore.weeklyForecast[i + 1].startTime
        ).toDateString() !== wDate
      ) {
        //newForecast.nightHumidity = `${w.relativeHumidity.value}%`
        newForecast.nightPrecipitation = `${
          w.probabilityOfPrecipitation.value !== null
      ...
      newForecast.nightIcon = w.icon;
      newForecast.nightDetailed = w.detailedForecast;
      newForecast.highestChanceOfRain = `${Math.max(
        Number(newForecast.dayPrecipitation.slice(0, -1)),
        Number(newForecast.nightPrecipitation.slice(0, -1))
      )}%`;
